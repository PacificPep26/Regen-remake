...
export async function middleware(request: NextRequest) {
  ...
  if (urlHasCountry) {
    // 22+ / research-use-only entry gate — mirrors the WordPress site's
    // inc/gate.php: intercept every frontend request until a signed
    // acknowledgment cookie is set, except the gate page itself.
    const pathAfterCountry =
      request.nextUrl.pathname.slice(`/${country}`.length) || "/"
    const isGatePath = pathAfterCountry.startsWith("/gate")
    const gateAcked = request.cookies.get("regenx_gate_ack")?.value === "1"

    if (!isGatePath && !gateAcked) {
      const target = request.nextUrl.pathname + (request.nextUrl.search || "")
      const gateUrl = `${request.nextUrl.origin}/${country}/gate?redirect=${encodeURIComponent(
        target
      )}`
